...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    List<Product> findAllByProductCategoryId(int id);

    @Query("SELECT p FROM Product p INNER JOIN p.productCategory c WHERE CONCAT(p.name, p.price, c.name, p.description) LIKE %?1% ")
